...
class RoiTableDialog(QDialog):
    ...
    def __init__(self, parent, rois: list,
        ...
        filter_frame.setStyleSheet(
            f"QFrame{{background:{dc('#1e1e2e', '#f2f2f2')};border-radius:5px;padding:2px;}}"
            f"QLabel{{color:{dc('#ccc', '#1a1a1a')};font-size:11px;}}"
            f"QCheckBox{{color:{dc('#ddd', '#1a1a1a')};font-size:11px;}}"
        ...
        def _sep():
            s = QLabel("│"); s.setStyleSheet(f"color:{dc('#555', '#c0c0c0')};")
            return s
        ...
        self._info_lbl.setStyleSheet(f"font-size:11px;color:{dc('#aaa', '#555555')};padding:2px;")
    ...
    def _rebuild(self):
        ...
        if not col_defs:
            ...
            lbl.setStyleSheet(f"color:{dc('#888', '#555555')};font-size:13px;padding:40px;")
...
class _PlotStyleBar(QWidget):
    ...
    def _swatch(self):
        self.btn_color.setStyleSheet(
            f"background:{self._color};border:1px solid {dc('#888', '#c0c0c0')};border-radius:3px;")
...
class _RegressionDialog(QDialog):
    ...
    def __init__(self, parent, rois, idx, cols):
        ...
        self._note.setStyleSheet(f"color:{dc('#aaa', '#555555')};font-size:11px;padding:2px;")
