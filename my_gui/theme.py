...
def dc(dark: str, light: str) -> str:
    """Theme-aware colour picker for use inside widget stylesheets.

    Returns ``dark`` in dark mode and ``light`` in light mode, so hardcoded
    colours adapt to the active theme while dark mode stays byte-identical::

        from my_gui.theme import dc
        w.setStyleSheet(f"background: {dc('#1c1c1c', '#ffffff')}; "
                        f"color: {dc('#e0e0e0', '#1a1a1a')};")

    Accent / identity colours (oranges, greens, blues, the #1565c0 highlight,
    white/black on an accent) should be left as-is — they read on both themes.
    """
    return dark if _current_mode == "dark" else light


# Convenience wrappers for the most common neutral roles (dark value first,
# so dark mode is unchanged; light value chosen for contrast on a light ground).
def card_bg() -> str:      return dc("#1c1c1c", "#ffffff")   # panels / cards
def input_bg() -> str:     return dc("#2a2a2a", "#ffffff")   # line edits / combos / lists
def neutral_text() -> str: return dc("#e0e0e0", "#1a1a1a")   # primary text
def subtle_text() -> str:  return dc("#aaaaaa", "#555555")   # secondary text
def faint_text() -> str:   return dc("#666666", "#888888")   # hints / placeholders
def neutral_border() -> str: return dc("#444444", "#c0c0c0") # borders / separators
def neutral_btn_bg() -> str: return dc("#3a3a3a", "#e6e6e6") # neutral (non-accent) buttons
