...
def _grp_style() -> str:
    """GroupBox stylesheet, evaluated at widget-build time so dc() picks the
    active theme (the module constant would resolve at import, before
    apply_theme())."""
    return (
        "QGroupBox {"
        f"  border: 1px solid {dc('#3a3a3a', '#c0c0c0')};"
        "  border-radius: 5px;"
        "  margin-top: 6px;"
...
class HpcTab(QWidget):
    ...
    def __init__(self):
        ...
        scroll.setStyleSheet(
            ...
            "QScrollBar:vertical {"
            f"  background: {dc('#1e1e1e', '#f2f2f2')}; width: 8px;"
            "  border-radius: 4px; margin: 0px;"
            "}"
            "QScrollBar::handle:vertical {"
            f"  background: {dc('#555', '#c0c0c0')}; border-radius: 4px; min-height: 24px;"
            "}"
            f"QScrollBar::handle:vertical:hover {{ background: {dc('#888', '#555555')}; }}"
        ...
        try:
            ...
            _lbl_shell.setStyleSheet(
                f"color:{dc('#8b949e', '#555555')}; font-size:11px; padding-left:4px;"
            )
            _lt_bar.addWidget(_lbl_shell, stretch=1)

            _lt_bar_w = QWidget()
            _lt_bar_w.setStyleSheet(
                f"background:{dc('#161b22', '#f2f2f2')}; border-bottom:1px solid {dc('#30363d', '#c0c0c0')};"
            )
            ...
            _lbl_info.setStyleSheet(
                f"color:{dc('#c9d1d9', '#1a1a1a')}; font-family:Menlo, Consolas, 'DejaVu Sans Mono', 'Courier New'; "
            ...
            _btn_copy.setStyleSheet(
                f"QPushButton{{background:{dc('#21262d', '#e6e6e6')};color:{dc('#c9d1d9', '#1a1a1a')};border:1px solid {dc('#30363d', '#c0c0c0')};"
                "border-radius:5px;font-size:12px;}"
                f"QPushButton:hover{{background:{dc('#30363d', '#e6e6e6')};color:{dc('white', '#1a1a1a')};}}"
            )
    ...
    def _build_actions_group(self) -> QGroupBox:
        ...
        note.setStyleSheet(f"font-size: 11px; color: {dc('#888', '#555555')};")
