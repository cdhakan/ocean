...
# Theme-aware colour picker: dc(dark, light) → dark in dark mode, light in light.
from my_gui.theme import dc
...
# Built lazily (a function, not a module constant) so dc() is evaluated at
# widget-build time — after apply_theme() has set the active mode — rather than
# at import time when the mode may not be known yet.
def _group_box_style() -> str:
    return f"""
    QGroupBox {{ border: 1px solid {dc('#444', '#c0c0c0')}; border-radius: 6px;
    ...
    QGroupBox::title {{ subcontrol-origin: padding; subcontrol-position: top left;
                       ...
                       font-size: 17px; color: {dc('#e6e6e6', '#1a1a1a')}; }}
"""
...
class _ScanCard(QWidget):
    ...
    def __init__(self, key: str, name: str,
        ...
        scan_lbl.setStyleSheet(f"font-size: 12px; color: {dc('#ccc', '#1a1a1a')}; border: none;")
        ...
        self.combo.setStyleSheet(
            f"QComboBox {{ background: {dc('#2a2a2a', '#ffffff')}; color: {dc('#ddd', '#1a1a1a')}; border: 1px solid {dc('#555', '#c0c0c0')};"
            ...
            f"  background: {dc('#2a2a2a', '#ffffff')}; color: {dc('#eee', '#1a1a1a')};"
            f"  border: 1px solid {dc('#666', '#c0c0c0')}; border-radius: 4px; outline: none;"
        ...
        self.btn_clear.setStyleSheet(
            f"QPushButton {{ background: {dc('#3a3a3a', '#e6e6e6')}; color: {dc('#aaa', '#555555')}; border: 1px solid {dc('#555', '#c0c0c0')};"
            f"  border-radius: 5px; font-size: 11px; padding: 2px 8px; }}"
            f"QPushButton:hover {{ background: {dc('#555', '#e6e6e6')}; color: {dc('white', '#1a1a1a')}; }}"
        )
        ...
        self.lbl_path.setStyleSheet(
            f"QLabel {{ font-size: 11px; color: {dc('#666', '#888888')}; background: transparent; border: none; }}"
        )
        ...
        self._btn_view_path.setStyleSheet(
            f"QPushButton {{ background: {dc('#2a2a2a', '#e6e6e6')}; color: {dc('#aaa', '#555555')}; border: 1px solid {dc('#444', '#c0c0c0')};"
            f"  border-radius: 4px; font-size: 11px; padding: 0px; }}"
            f"QPushButton:hover {{ background: {dc('#444', '#e6e6e6')}; color: {dc('white', '#1a1a1a')}; }}"
        )
    ...
    def _set_path(self, path: str):
        ...
        if path:
            ...
            self.lbl_path.setStyleSheet(
                f"QLabel {{ font-size: 11px; color: {dc('#666', '#888888')}; "
...
class _B1VFAScanCard(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        frame.setStyleSheet(
            ...
            f"  border-radius: 10px; background: {dc('#1c1c1c', '#ffffff')}; }}"
        )
        ...
        divider.setStyleSheet(f"QFrame {{ background: {dc('#333', '#c0c0c0')}; border: none; max-width: 1px; }}")
...
class ScanDirTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.edit_study.setStyleSheet(
            f"QLineEdit {{ background: {dc('#2a2a2a', '#ffffff')}; color: {dc('#ccc', '#1a1a1a')}; "
            f"border: 1px solid {dc('#555', '#c0c0c0')}; border-radius: 4px; padding: 4px 8px; }}"
        )
        ...
        self.lbl_study_meta.setStyleSheet(f"font-size: 11px; color: {dc('#888', '#555555')};")
        meta_row.addWidget(self.lbl_study_meta, stretch=1)

        self.btn_refresh = QPushButton("↻  Refresh")
        self.btn_refresh.setFixedHeight(26)
        self.btn_refresh.setStyleSheet(
            f"QPushButton {{ background: {dc('#333', '#e6e6e6')}; color: {dc('#aaa', '#555555')}; border: none; "
        ...
        self.lst_scan_list.setStyleSheet(f"""
            QListWidget {{
                background: {dc('#1a1a1a', '#ffffff')};
                color: {dc('#ddd', '#1a1a1a')};
                border: 1px solid {dc('#333', '#c0c0c0')};
                border-radius: 4px;
                padding: 4px;
                font-family: Arial;
                font-size: 13px;
        ...
        self.lbl_gs_info.setStyleSheet(f"font-size: 11px; color: {dc('#888', '#555555')};")
        ...
        self.lst_gs_scans.setStyleSheet(f"""
            QListWidget {{ background: {dc('#1a1a1a', '#ffffff')}; color: {dc('#ddd', '#1a1a1a')}; border: 1px solid {dc('#333', '#c0c0c0')};
            ...
            QListWidget::item:hover:!selected {{ background: {dc('#2a2a2a', '#e6e6e6')}; }}
        """)
        ...
        self.lbl_gs_scan_detail.setStyleSheet(f"font-size: 10px; color: {dc('#888', '#555555')};")
        ...
        self.lbl_mrd_info.setStyleSheet(f"font-size: 11px; color: {dc('#888', '#555555')};")
