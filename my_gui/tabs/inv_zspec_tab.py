...
class InvZSpecTab(QWidget):
    ...
    def _build_left(self) -> QScrollArea:
        ...
        btn_scan.setStyleSheet(
            f"background:{dc('#555','#e6e6e6')};color:{dc('white','#1a1a1a')};"
        ...
        self.lbl_inv_pools.setStyleSheet(f"font-size: 11px; color: {dc('#aaa','#555555')};")
    ...
    def _show_roi_spectra(self):
        ...
        for _key, _lbl, _col in _chk_items:
            ...
            if _key not in ('raw', 'sum'):
                ...
                sw.setStyleSheet(f"background:{_col};border:1px solid {dc('#888','#c0c0c0')};border-radius:3px;")
                ...
                def _pick_color(_checked=False, k=_key, s=sw, c=chk):
                    ...
                    if col.isValid():
                        hexc = col.name()
                        _pool_colors[k] = hexc
                        s.setStyleSheet(f"background:{hexc};border:1px solid {dc('#888','#c0c0c0')};"
                                        f"border-radius:3px;")
        ...
        def _populate_quesp(rname: str):
            ...
            if qfig is None:
                ...
                lbl.setStyleSheet(f"color:{dc('#888','#555555')}; padding:6px;")
                q_lay.addWidget(lbl)
