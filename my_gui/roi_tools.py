...
def choose_background_image(parent, scan_paths, pv360: bool = False):
    ...
    if lst.count() == 0:
        ...
        _hint.setStyleSheet(f"color:{dc('#999', '#555555')};font-size:11px;")
        lay.addWidget(_hint)
...
class ROIPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self._stats_label.setStyleSheet(f"font-size: 10px; color: {dc('gray', '#555555')};")
        ...
        self._edit_shape_btn.setStyleSheet(
            f"QPushButton {{ border: 1px solid {dc('#555', '#c0c0c0')}; border-radius: 4px; }}"
            "QPushButton:checked { background: #aa6600; color: white; font-weight: bold; }"
            f"QPushButton:hover   {{ background: {dc('#444', '#e6e6e6')}; }}"
        )
