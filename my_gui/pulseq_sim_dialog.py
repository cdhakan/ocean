...
class PulseqSimDialog(QDialog):
    ...
    def _init_theme(self):
        ...
        self.setStyleSheet(
            ...
            f"QTabBar::tab:selected {{ background:#1565c0; color:white; font-weight:bold; }}"
        )
    ...
    def _on_dark_toggled(self, checked: bool):
        """Re-theme every existing figure across the tabs to match the toggle."""
        self._dark_fig = bool(checked)
        from my_gui.fig_theme import apply_fig_dark_theme
        # Query live FigureCanvas widgets from the dialog's widget tree rather
        # than caching each tab's layout — a cached layout reference can outlive
        # the underlying C++ object once a tab's figures are rebuilt, which
        # raised 'wrapped C/C++ object of type QVBoxLayout has been deleted'.
        for w in self.findChildren(FigureCanvas):
            try:
                apply_fig_dark_theme(w.figure, self._dark_fig)
                w.draw()
            except RuntimeError:
                pass   # canvas mid-teardown — skip
    ...
    def _build_ui(self):
        ...
        self.btn_view.setStyleSheet(
            ...
            f"QPushButton:disabled{{background:{dc('#bbb', '#e6e6e6')};color:{dc('#eee', '#888888')};}}")
    ...
    def _view_sequence(self):
        ...
        self.lbl_status.setStyleSheet(f"font-size:11px; color:{dc('#888', '#555555')}; background:transparent;")
