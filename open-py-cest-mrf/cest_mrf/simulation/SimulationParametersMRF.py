    ...
    def _transform_dict(self, dictionary):
        ...
        for k, v in dictionary.items():
            ...
            if 't' == par:
                ...
                # Guard against T=0 entries in the parameter grid.
                # T=0 is physically undefined (R = 1/T → ∞); replace with np.inf
                # so BMCTool receives a finite-or-inf rate rather than triggering
                # a RuntimeWarning that can mask real issues downstream.
                zero_mask = arr == 0.0
                if zero_mask.any():
                    import warnings
                    bad_indices = np.flatnonzero(zero_mask).tolist()
                    warnings.warn(
                        f"SimulationParametersMRF: parameter '{k}' contains "
                        f"{zero_mask.sum()} zero value(s) at grid indices "
                        f"{bad_indices[:10]}{'…' if len(bad_indices) > 10 else ''}. "
                        f"T=0 is physically undefined; those entries will have R=inf "
                        f"and may produce NaN fingerprints — check your parameter grid.",
                        UserWarning,
                        stacklevel=2,
                    )
                with np.errstate(divide='ignore'):
                    n_v = np.where(zero_mask, np.inf, 1.0 / arr)
                if pool == 'cest_pool':
                    pool_id = k[4:]
