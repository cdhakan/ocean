        ...
        for block_counter in range(len(self.dict_block_events)):
            ...
            if is_valid:
                ...
                if hasattr(block, 'rf'):
                    ...
                    t = rf.t + rf.delay
                    tc = tc + rf.delay
                    # Pad the RF envelope with zeros at both ends so short,
                    # high-amplitude pulses (e.g. 0.1 ms hard / spin-lock pulses)
                    # render as visible vertical spikes rather than a sub-pixel
                    # horizontal dash that disappears on a long time axis.
                    _rt = np.concatenate(([t[0]], t, [t[-1]]))
                    _ra = np.concatenate(([0.0], np.abs(rf.signal), [0.0]))
                    sp12.plot(t_factor * (t0 + _rt), _ra)
