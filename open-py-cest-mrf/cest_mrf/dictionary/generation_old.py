...
def save_dict_mat(dict_fn, dictionary):
    mat_dict = {k: np.array(v) if isinstance(v, list) else v
                for k, v in dictionary.items()}
    hdf5storage.savemat(dict_fn, mat_dict, matlab_compatible=True)
    print("Dictionary saved to {dict_fn} .")
...
def generate_mrf_cest_dictionary(seq_fn=None,
    ...
    save_dict_mat(dict_fn, dictionary)
